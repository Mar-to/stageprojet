        ...
        $formMapper
            ...
            ->tab('Fiche détail 2')
                ->with("Contenu de la Fiche détail (panneau qui s'affiche lors d'un click sur un marker)",
                        ...
                        <h2 style='margin: 0 0 25px 0'>
                          Contenu de la fiche détail
                          <span class='btn btn-primary' id='generate-body-template2' onclick='generateBodyTemplate2()'>Générer automatiquement le contenu de la fiche détail 2</span>
                        </h2>"])

                    ->add('infobar.headerTemplateUseMarkdown2', CheckboxType::class, ['label' => 'Utiliser la syntaxe markdown pour le header (sinon uniquement la syntaxe Nunjucks)', 'attr' => ['class' => 'use-markdown'], 'required' => false])
                    ->add('infobar.headerTemplate2', null, ['label' => 'En tête de la fiche (header)', 'attr' => ['class' => 'gogo-code-editor2', 'format' => 'twig', 'height' => '200'], 'required' => false])
                    ->add('infobar.bodyTemplateUseMarkdown2', CheckboxType::class, ['label' => 'Utiliser la syntaxe markdown pour le body (sinon uniquement la syntaxe Nunjucks)', 'attr' => ['class' => 'use-markdown'], 'required' => false])
                    ->add('infobar.bodyTemplate2', null, ['label' => 'Corps de la fiche (body)', 'attr' => ['class' => 'gogo-code-editor2', 'data-id' => 'body-template2', 'format' => 'twig', 'height' => '500'], 'required' => false])
                ->end()
                ->with('Autres Paramètres')
                    ->add('infobar.width2', IntegerType::class, ['label' => 'Largeur de la fiche détail (en pixels, par défaut : 540)', 'required' => false])
                ->end()
            ->end()
            ->tab('Liste des Champs disponibles (aide)')
                ...
                ->end()
            ->end()
            ->tab('Liste des Champs disponibles (2eme formulaire)')
                ->with('')
                    ->add('elementFormFieldsJson2', HiddenType::class, ['attr' => ['class' => 'gogo-form-fields', 'dataproperties' => $propertiesText]])
                ->end()
            ->end()
