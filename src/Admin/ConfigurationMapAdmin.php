        ...
        $formMapper
            ->tab('Paramètres de la carte')
                ->with('La carte')
                    ...
                    ->add('defaultViewPicker', HiddenType::class, array('mapped' => false, 'attr' => [
                ...
                ->end()
                ->with('Cookies')
                    ->add('saveViewportInCookies', CheckboxType::class, array('label' => "Sauvegarder la position courante de la carte dans les cookies", 'required' => false))
                    ->add('saveTileLayerInCookies', CheckboxType::class, array('label' => "Sauvegarder le choix du fond de carte par l'utilisateur dans les cookies", 'required' => false))
                ->end()
            ->end()
            ->tab('Fonctionnalités')
            ...
            ->tab('Menu')
                ->with('Menu (contient les filtre des catégories et la barre de recherche)')
                    ->add('menu.width', IntegerType::class, array('label' => "Largueur du menu", 'required' => false, 'attr' => ['placeholder' => '290']))
                    ->add('menu.smallWidthStyle', CheckboxType::class, array('label' => "Utiliser un style compréssé", 'label_attr' => ['title' => "Diminue un peu la taille de la police et les marges. Pratique lorsque le nom des catégories sont longues que l'on veut gagner en largeur"], 'required' => false))
                    ->add('menu.showOnePanePerMainOption', CheckboxType::class, array('label' => "Afficher un sous menu pour chaque catégorie principale", 'required' => false))
                    ->add('menu.showCheckboxForMainFilterPane', CheckboxType::class, array('label' => "Afficher les checkbox dans la panneau principal", 'required' => false))
                    ->add('menu.showCheckboxForSubFilterPane', CheckboxType::class, array('label' => "Afficher les checkbox dans les sous panneaux",  'label_attr' => ['title' => "Valable uniquement si \"afficher un sous menu pour chaque catégorie principale\" est coché"], 'required' => false))
                    ->add('menu.displayNumberOfElementForEachCategory', CheckboxType::class, array('label' => "Pour chaque catégorie, afficher le nombre d'élément ayant cette catégorie", 'required' => false))
                    ->add('menu.displayNumberOfElementRoundResults', CheckboxType::class, array('label' => "Arrondir les résultat (afficher 300+ au lieu de 326)",  'label_attr' => ['title' => "Valable uniquement si \"afficher le nombre d'éléments par catégorie\" est coché"], 'required' => false))
                ->end()
            ->end()
        ;
    }
