...
import { GoogleMapsGeocoded, GoogleMapsGeocodeQuery, GoogleMapsReverseQuery, ProviderHelpers, defaultProviderOptions, } from "./..";
import AdminLevel from "../../AdminLevel";
import { ResponseError } from "../../error";
import { decodeBase64, decodeUrlSafeBase64, encodeUrlSafeBase64, filterUndefinedObjectValues, getRequireFunc, isBrowser, } from "../../utils";
var GoogleMapsProvider = /** @class */ (function () {
    ...
    GoogleMapsProvider.prototype.geocode = function (query, callback, errorCallback) {
        ...
        var geocodeQuery = ProviderHelpers.getGeocodeQueryFromParameter(query, GoogleMapsGeocodeQuery);
    ...
    GoogleMapsProvider.prototype.geodecode = function (latitudeOrQuery, longitudeOrCallback, callbackOrErrorCallback, errorCallback) {
        var _this = this;
        var _a, _b;
        var reverseQuery = ProviderHelpers.getReverseQueryFromParameters(latitudeOrQuery, longitudeOrCallback, GoogleMapsReverseQuery);
    ...
    GoogleMapsProvider.mapToGeocoded = function (result) {
            ...
            addressComponent.types.forEach(function (type) {
                switch (type) {
                    ...
                    case "sublocality_level_5":
                        subLocalityLevels.push(AdminLevel.create({
                            level: parseInt(type.substr(-1), 10),
                            name: addressComponent.long_name,
                            code: addressComponent.short_name,
                        }));
        ...
        var geocoded = GoogleMapsGeocoded.create({
            coordinates: {
                latitude: latitude,
                longitude: longitude,
            },
    ...
    GoogleMapsProvider.signQuery = function (secret, pathname, params) {
        var crypto = getRequireFunc()("crypto");
        var filteredRequestParams = filterUndefinedObjectValues(params);
        var safeSecret = decodeBase64(decodeUrlSafeBase64(secret));
        var toSign = pathname + "?" + new URLSearchParams(filteredRequestParams).toString();
        var hashedSignature = encodeUrlSafeBase64(crypto.createHmac("sha1", safeSecret).update(toSign).digest("base64"));
